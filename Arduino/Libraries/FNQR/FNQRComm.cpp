...
void Communication::HandleOrder(byte inData[], OrderSource orderSource)
{
	...
	else if (inData[1] == Orders::requestMoveLeg)
	{
		switch (inData[2]) {
		case 1:
			robotAction.robot.leg1.MoveToRelatively(Point(inData[3] - 64, inData[4] - 64, inData[5] - 64));
			break;
		case 2:
			robotAction.robot.leg2.MoveToRelatively(Point(inData[3] - 64, inData[4] - 64, inData[5] - 64));
			break;
		case 3:
			robotAction.robot.leg3.MoveToRelatively(Point(inData[3] - 64, inData[4] - 64, inData[5] - 64));
			break;
		case 4:
			robotAction.robot.leg4.MoveToRelatively(Point(inData[3] - 64, inData[4] - 64, inData[5] - 64));
			break;
		}
		outData[outDataCounter++] = Orders::orderDone;
	}
...
void UpdateService()
{
	...
	if (communication != NULL) {
		communication->robotAction.robot.UpdateAction();
